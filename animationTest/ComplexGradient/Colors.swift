...
extension Color{
    init(averageOf colors: [Color]){
        var uiColors: [UIColor] = []
        for color in colors{
            var uiColor: UIColor
            do{
                 try uiColor = color.convertToUIColor()
            }catch{
                fatalError("cant convert color \(color.description) to UIColor")
            }
            uiColors.append(uiColor)
        }
        self.init(UIColor(averageOf: uiColors))
    }
}
    
extension UIColor{
    convenience init(averageOf colors: [UIColor]){
        guard colors.count > 0 else {fatalError("no colors to calculate average")}
        let colorWeight = CGFloat(1) / CGFloat(colors.count)
        var red: CGFloat = 0
        var green: CGFloat = 0
        var blue: CGFloat = 0
        var alpha: CGFloat = 0
        for color in colors{
            ...
            var thiAlpha: CGFloat = 0
            color.getRed(&thisRed, green: &thisGreen, blue: &thisBlue, alpha: &thiAlpha)
            red += thisRed * colorWeight
            green += thisGreen * colorWeight
            blue += thisBlue * colorWeight
            alpha += thiAlpha * colorWeight
        }
